 ...
 */
extern char rf_crc;
extern char rf_addr_width;
extern char rf_speed_power;
extern char rf_channel;

/* Status variable updated every time SPI I/O is performed */
extern char rf_status;
...
void w_rx_addr(char pipe, char *addr);  // Configure RX address of "rf_addr_width" size into the specified pipe
void w_tx_payload(char len, char *data);
void w_tx_payload_noack(char len, char *data);  /* Only used in auto-ack mode with RF24_EN_DYN_ACK enabled;
						 * send this packet with no auto-ack.
						 */
char r_rx_peek_payload_size();  // Peek size of incoming RX payload
...
void w_ack_payload(char pipe, char len, char *data);  // Used when RF24_EN_ACK_PAY is enabled to manually ACK a received packet
...
void msprf24_init();  /* Set the various configuration variables before running this.
		       ...
		       */
void msprf24_close_pipe(char pipeid);       // Disable specified RX pipe
void msprf24_close_pipe_all();                       // Disable all RX pipes (used during initialization)
void msprf24_open_pipe(char pipeid, char autoack); // Enable specified RX pipe, optionally turn auto-ack (Enhanced ShockBurst) on
char msprf24_pipe_isopen(char pipeid); // Check if specified RX pipe is active
void msprf24_set_pipe_packetsize(char pipe, char size);  // Set static length of pipe's RX payloads (1-32), size=0 enables DynPD.
void msprf24_set_retransmit_delay(int us);           // 500-4000uS range, clamped by RF speed
void msprf24_set_retransmit_count(char count);       // 0-15 retransmits before MAX_RT (RF24_IRQ_TXFAILED) IRQ raised
char msprf24_get_last_retransmits();        // # times a packet was retransmitted during last TX attempt
char msprf24_get_lostpackets();      /* # of packets lost since last time the Channel was set.
	                                       * Running msprf24_set_channel() without modifying rf_channel will reset this counter.
                                               */
char msprf24_is_alive();                    // Hello World, test if chip is present and/or SPI is working.
...
void msprf24_enable_feature(char feature);    /* Enable specified feature (RF24_EN_* from nRF24L01.h, except RF24_EN_CRC) */
void msprf24_disable_feature(char feature);   /* Disable specified feature                                                */

// Change chip state and activate I/O
char msprf24_current_state();    // Get current state of the nRF24L01+ chip, test with RF24_STATE_* #define's
...
char msprf24_queue_state();      // Read FIFO_STATUS register; user should compare return value with RF24_QUEUE_* #define's
char msprf24_scan();             // Scan current channel for RPD (looks for any signals > -64dBm)

// IRQ handling
char msprf24_rx_pending();		   /* Query STATUS register to determine if RX FIFO data is available for reading. */
char msprf24_get_irq_reason();            /* Query STATUS register for the IRQ flags, test with RF24_IRQ_* #define's
						    ...
						    */
void msprf24_irq_clear(char irqflag);     /* Clear specified Interrupt Flags (RF24_IRQ_* #define's) from the transceiver.
