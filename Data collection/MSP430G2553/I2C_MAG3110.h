...
void I2C_transmit(int registerAddr, int value){
    ...
    while((IFG2 & UCB0TXIFG) == 0); // wait for UCBOTXIFG to set
    UCB0TXBUF = registerAddr;   // address of the control register
    while((IFG2 & UCB0TXIFG) == 0); // wait for transmission to finish, UCB0TXIFG will be set then
...
int I2C_receive(int registerAddr){
    ...
    while((IFG2 & UCB0TXIFG) == 0); // wait for transmission to finish
