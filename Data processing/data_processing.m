...
x1 = xlsread('Book5',2,'A:A'); % reading x,y,z data for both sensors
x1 = typecast(uint16(x1),'int16');  % converting data to int16 form uint16
y1 = xlsread('Book5',2,'B:B');
y1 = typecast(uint16(y1),'int16');
z1 = xlsread('Book5',2,'C:C');
z1 = typecast(uint16(z1),'int16');
x2 = xlsread('Book5',2,'D:D');
x2 = typecast(uint16(x2),'int16');
y2 = xlsread('Book5',2,'E:E');
y2 = typecast(uint16(y2),'int16');
z2 = xlsread('Book5',2,'F:F');
z2 = typecast(uint16(z2),'int16');
distance = xlsread('Book5',2,'G:G'); % in cm
